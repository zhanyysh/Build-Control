...
from app.models.models import Task, TaskBase, User, UserRole, TaskStatus, Project
...
def read_tasks(project_id: Optional[int] = None, current_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    if current_user.role == UserRole.SYSTEM_ADMIN:
        ...
        statement = select(Task).join(Project).where(Project.company_id == current_user.company_id)
...
def delete_task(task_id: int, current_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    if current_user.role not in [UserRole.ADMIN, UserRole.FOREMAN, UserRole.SYSTEM_ADMIN]:
